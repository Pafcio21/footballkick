...
try {
    $conn = new PDO("mysql:host=$servername;dbname=$dbname",$username, $password);
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $conn->prepare("SELECT round, team1_id, team2_id, tournament_id FROM games");
    $stmt->execute();
    $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
    $allTeams=$stmt->fetchAll();
}   catch(PDOException $e){    
...
foreach($matches as $round1 => $roundMatches1)
{
    ...
    foreach($roundMatches1 as $match1) {
        $sql = "INSERT INTO games(round, tournament_id, team1_id, team2_id) Values('$round1', '$tournament_id', '" . $match1[0] . "', '" . $match1[1] . "')";
        $conn->exec($sql);
    }

   
}
header("Location: allTournaments.php");
exit();